...
export default {
  name: 'iscroll',
  props: ['contentStyle', 'options', 'disabled', 'downRefreshabled', 'disabledDefaultRefresh', 'scrollWrapStyle'],
  data () {
    return {
    ...
    */
    this.$nextTick(function () { // nextTick在全局VUE DOM結點更新是會調用
      ...
      if (scrollWrap) {
        ...
        IScroll.on('bottomBounce', function () { // 需要在 上拉超过底部并松手一瞬间监控刷新，scrollEnd则是滑动结束后再执行, 因此不使用scrollEnd
        ...
        IScroll.on('topBounce', function (exceedHeight) { // 需要在 下拉超过底部并松手一瞬间监控刷新，scrollEnd则是滑动结束后再执行, 因此不使用scrollEnd
          if (IScroll.isInTransition) { // 当触发bottomBounce是非手动touch操作，而是惯性transition导致，则不处理
            return
          }
          if (self.downRefreshabled && exceedHeight > DOWN_REFRESH_EXCCED_HEIGHT) {
            self.$emit('refresh')
            if (!self.disabledDefaultRefresh) {
